...
const Adminpage = () => {
  ...
  // 🔥 COPY SINGLE USER
  const copySingleUser = (user) => {
    const userData = `Name: ${user.name}\nPhone: ${user.phone}\nEmail: ${user.email}\nPlace: ${user.place}\nCountry: ${user.country}`;
    
    navigator.clipboard.writeText(userData).then(() => {
      setCopyStatus(`✅ Copied ${user.name}`);
  ...
  const copyAllUsers = () => {
    if (users.length === 0) {
      setCopyStatus("No users to copy");
      return;
    }
    ...
    navigator.clipboard.writeText(allData).then(() => {
      setCopyStatus(`✅ Copied ${users.length} users`);
      setTimeout(() => setCopyStatus(""), 2000);
    }).catch(() => {
      setCopyStatus("❌ Copy failed");
    });
          ...
          <Col lg={12}>
            ...
            {filteredUsers.length === 0 ? (
              ...
              <div className="bg-white rounded-4 shadow-xl overflow-hidden border-0">
                <div className="p-4 bg-gradient-primary text-white">
                  <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <h5 className="mb-0 fw-bold text-white"> {/* ✅ FIXED WHITE TEXT */}
                      <i className="bi bi-people-fill me-2"></i>
                      Users Registrations ({filteredUsers.length})
                    </h5>
                    <div className="d-flex gap-2 flex-wrap">
                          ...
                          <td className="pe-4 align-middle py-4 text-center">
                            <div className="dropdown-container">
                              <DropdownButton
                                ...
                                size="sm"
                              >
                                {/* 🔥 NEW COPY OPTION - TOP OF DROPDOWN */}
                                <Dropdown.Item
                                  onClick={() => copySingleUser(user)}
                                  className="dropdown-item-custom text-primary fw-bold"
                                >
                                  <i className="bi bi-copy me-2 text-primary"></i>
                                  Copy Details
                                </Dropdown.Item>
